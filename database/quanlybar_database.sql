...
DROP DATABASE IF EXISTS quanly_bar;
...
CREATE TABLE cua_hang (
    ...
    trang_thai      TINYINT DEFAULT 1 COMMENT '1=Hoạt động, 0=Ngừng',
...
CREATE TABLE tai_khoan (
    ...
    email           VARCHAR(100),
    dien_thoai      VARCHAR(20),
    vai_tro         ENUM('admin', 'quan_ly', 'thu_ngan', 'phuc_vu', 'bep', 'kho') NOT NULL DEFAULT 'phuc_vu',
    cua_hang_id     INT,
...
CREATE TABLE phan_quyen (
    ...
    ma_chuc_nang    VARCHAR(50) NOT NULL COMMENT 'VD: HE_THONG, HOAT_DONG, KHO_HANG...',
    ...
    UNIQUE KEY uk_taikhoan_chucnang (tai_khoan_id, ma_chuc_nang)
) ENGINE=InnoDB;

-- Bảng Thiết bị đăng nhập (theo dõi thiết bị: Desktop, POS, Mobile)
CREATE TABLE thiet_bi (
    id              INT AUTO_INCREMENT PRIMARY KEY,
    ten_thiet_bi    VARCHAR(100) NOT NULL COMMENT 'VD: LAPTOP-6KP13OSJ',
    loai_thiet_bi   ENUM('desktop', 'mobile', 'pos', 'tablet') NOT NULL,
    ma_thiet_bi     VARCHAR(255) COMMENT 'Device ID / MAC address',
    cua_hang_id     INT,
...
CREATE TABLE ban (
    id              INT AUTO_INCREMENT PRIMARY KEY,
    ten_ban         VARCHAR(50) NOT NULL COMMENT 'VD: Bàn 01, Bàn 02...',
    khu_vuc_id      INT NOT NULL,
    nhom_hien_thi   VARCHAR(100) COMMENT 'Nhóm hiển thị trên sơ đồ',
    loai_phong      VARCHAR(50) COMMENT 'VD: Phòng VIP, Phòng thường',
    so_cho          INT DEFAULT 4,
    trang_thai      ENUM('trong', 'dang_su_dung', 'dat_truoc', 'don_dep') DEFAULT 'trong',
    ghi_chu         TEXT,
...
CREATE TABLE nhom_mat_hang (
    id              INT AUTO_INCREMENT PRIMARY KEY,
    ten_nhom        VARCHAR(100) NOT NULL COMMENT 'VD: BÒ-BÊ-TRÂU-DÊ, CÁC MÓN CÁ, ĐỒ UỐNG CÁC LOẠI...',
    nhom_cha_id     INT COMMENT 'Hỗ trợ phân cấp nhóm',
...
CREATE TABLE don_vi_tinh (
    id              INT AUTO_INCREMENT PRIMARY KEY,
    ten_dvt         VARCHAR(50) NOT NULL COMMENT 'VD: đĩa, lít, bát, kg, cái, bìa, chai, suất, gói, nồi, bao, mâm',
    trang_thai      TINYINT DEFAULT 1
) ENGINE=InnoDB;
...
CREATE TABLE mat_hang (
    id              INT AUTO_INCREMENT PRIMARY KEY,
    ma_hang         VARCHAR(50) UNIQUE COMMENT 'Mã hàng tự sinh hoặc tùy chỉnh',
    ten_hang        VARCHAR(200) NOT NULL COMMENT 'VD: Quýt tráng miệng, Rượu trắng, Gà HMông...',
    nhom_mat_hang_id INT,
    dvt_id          INT COMMENT 'Đơn vị tính mặc định',
    gia_ban         DECIMAL(15,2) DEFAULT 0 COMMENT 'Giá bán lẻ',
    gia_von         DECIMAL(15,2) DEFAULT 0 COMMENT 'Giá vốn / giá nhập',
    hinh_anh_url    VARCHAR(500),
    mo_ta           TEXT,
    la_combo        TINYINT DEFAULT 0 COMMENT '1=Là combo nhiều món',
    co_dinh_luong   TINYINT DEFAULT 0 COMMENT '1=Có định lượng nguyên liệu',
    trang_thai      ENUM('con_mon', 'het_hang', 'ngung_ban') DEFAULT 'con_mon',
    thu_tu          INT DEFAULT 0,
    ngay_tao        DATETIME DEFAULT CURRENT_TIMESTAMP,
    nguoi_tao       VARCHAR(50),
    ngay_cap_nhat   DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE dinh_luong (
    id              INT AUTO_INCREMENT PRIMARY KEY,
    mat_hang_id     INT NOT NULL COMMENT 'Món ăn thành phẩm',
    nguyen_lieu_id  INT NOT NULL COMMENT 'Nguyên liệu cần dùng (cũng là mặt hàng)',
...
CREATE TABLE nhom_khach_hang (
    id              INT AUTO_INCREMENT PRIMARY KEY,
    ten_nhom        VARCHAR(100) NOT NULL COMMENT 'VD: VIP, Thường, Thân thiết',
    mo_ta           VARCHAR(500),
    trang_thai      TINYINT DEFAULT 1
) ENGINE=InnoDB;
...
CREATE TABLE ca_lam_viec (
    id              INT AUTO_INCREMENT PRIMARY KEY,
    ten_ca          VARCHAR(100) NOT NULL COMMENT 'VD: Ca sáng, Ca chiều, Ca tối',
    gio_bat_dau     TIME,
    gio_ket_thuc    TIME,
    ghi_chu         TEXT
) ENGINE=InnoDB;
...
CREATE TABLE nhan_vien (
    ...
    ho_ten          VARCHAR(100) NOT NULL,
    chuc_vu         VARCHAR(50) COMMENT 'VD: Quản lý, Thu ngân, Phục vụ, Bếp...',
    dien_thoai      VARCHAR(20),
    dia_chi         VARCHAR(500),
    email           VARCHAR(100),
    ngay_sinh       DATE,
    ...
    cua_hang_id     INT,
    tai_khoan_id    INT COMMENT 'Liên kết tài khoản đăng nhập',
    hinh_anh_url    VARCHAR(500),
    trang_thai      TINYINT DEFAULT 1 COMMENT '1=Đang làm, 0=Nghỉ việc',
    ghi_chu         TEXT,
    ngay_tao        DATETIME DEFAULT CURRENT_TIMESTAMP,
    ngay_cap_nhat   DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (cua_hang_id) REFERENCES cua_hang(id),
...
CREATE TABLE cham_cong (
    ...
    ngay            DATE NOT NULL,
    ca_lam_viec_id  INT,
    trang_thai      ENUM('khong_co_lich', 'di_lam', 'nghi_co_phep', 'nghi_khong_phep') NOT NULL DEFAULT 'di_lam',
    gio_vao         TIME,
    gio_ra          TIME,
    ghi_chu         VARCHAR(500),
    UNIQUE KEY uk_nhanvien_ngay (nhan_vien_id, ngay),
...
CREATE TABLE tam_ung_luong (
    ...
    ghi_chu         TEXT,
    trang_thai      ENUM('cho_duyet', 'da_duyet', 'da_hoan') DEFAULT 'cho_duyet',
    FOREIGN KEY (nhan_vien_id) REFERENCES nhan_vien(id)
) ENGINE=InnoDB;
...
CREATE TABLE bang_luong (
    id              INT AUTO_INCREMENT PRIMARY KEY,
    ten_bang_luong  VARCHAR(100) NOT NULL COMMENT 'VD: Bảng lương tháng 8/2026',
    thang           INT NOT NULL,
    nam             INT NOT NULL,
...
CREATE TABLE bang_luong_chi_tiet (
    ...
    tong_luong      DECIMAL(15,2) DEFAULT 0 COMMENT 'Lương thực lĩnh',
    ghi_chu         TEXT,
...
CREATE TABLE hoa_don (
    id              INT AUTO_INCREMENT PRIMARY KEY,
    so_hoa_don      VARCHAR(20) NOT NULL UNIQUE COMMENT 'VD: 082600001, 082600004',
    ban_id          INT,
    khach_hang_id   INT,
    nhan_vien_id    INT COMMENT 'Nhân viên phục vụ / thu ngân',
    cua_hang_id     INT,
    ngay_tao        DATETIME DEFAULT CURRENT_TIMESTAMP,
    gio_thanh_toan  TIME,
    so_phieu        VARCHAR(50),
    ...
    -- Tính tiền
    tien_hang       DECIMAL(15,2) DEFAULT 0 COMMENT 'Tổng tiền hàng chưa giảm',
    tien_giam_mat_hang DECIMAL(15,2) DEFAULT 0 COMMENT 'Giảm giá trên từng mặt hàng',
    tien_giam_tong_bill DECIMAL(15,2) DEFAULT 0 COMMENT 'Giảm giá tổng bill',
    phan_tram_giam  DECIMAL(5,2) DEFAULT 0,
    phi_dich_vu     DECIMAL(15,2) DEFAULT 0,
    thue_vat        DECIMAL(15,2) DEFAULT 0,
    tong_cong       DECIMAL(15,2) DEFAULT 0 COMMENT 'Số tiền phải thanh toán cuối cùng',

    -- Thanh toán
    tien_mat        DECIMAL(15,2) DEFAULT 0,
    chuyen_khoan    DECIMAL(15,2) DEFAULT 0,
    tien_the        DECIMAL(15,2) DEFAULT 0 COMMENT 'Thẻ ATM / tín dụng',
    voucher         DECIMAL(15,2) DEFAULT 0,
    the_tra_truoc   DECIMAL(15,2) DEFAULT 0 COMMENT 'Thẻ trả trước / prepaid',
    tru_tich_luy    DECIMAL(15,2) DEFAULT 0 COMMENT 'Trừ điểm tích lũy',
    dat_truoc       DECIMAL(15,2) DEFAULT 0 COMMENT 'Tiền đặt trước',
    con_no          DECIMAL(15,2) DEFAULT 0,

    trang_thai      ENUM('dang_phuc_vu', 'cho_thanh_toan', 'da_thanh_toan', 'huy') DEFAULT 'dang_phuc_vu',
    ghi_chu         TEXT,
    thiet_bi_id     INT COMMENT 'Thiết bị tạo hóa đơn',
    ngay_cap_nhat   DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    FOREIGN KEY (ban_id) REFERENCES ban(id),
    FOREIGN KEY (khach_hang_id) REFERENCES khach_hang(id),
    FOREIGN KEY (nhan_vien_id) REFERENCES nhan_vien(id),
...
CREATE TABLE hoa_don_chi_tiet (
    ...
    mat_hang_id     INT NOT NULL,
    ten_hang        VARCHAR(200) COMMENT 'Snapshot tên hàng tại thời điểm bán',
    dvt             VARCHAR(50) COMMENT 'Snapshot đơn vị tính',
    so_luong        DECIMAL(10,2) NOT NULL DEFAULT 1,
    don_gia         DECIMAL(15,2) NOT NULL DEFAULT 0,
    chiet_khau_phan_tram DECIMAL(5,2) DEFAULT 0 COMMENT 'CK% trên mặt hàng',
    tien_chiet_khau DECIMAL(15,2) DEFAULT 0,
    thanh_tien      DECIMAL(15,2) DEFAULT 0,
    ghi_chu         TEXT,
    trang_thai      ENUM('dang_cho', 'dang_che_bien', 'da_che_bien', 'da_phuc_vu', 'huy') DEFAULT 'dang_cho',
    thoi_gian_gui_bep DATETIME COMMENT 'Thời điểm In chế biến (F10)',
...
CREATE TABLE phuong_thuc_dat (
    id              INT AUTO_INCREMENT PRIMARY KEY,
    ten_phuong_thuc VARCHAR(100) NOT NULL COMMENT 'VD: Công văn, Điện thoại, Email, Tin nhắn, Trực tiếp'
) ENGINE=InnoDB;
...
CREATE TABLE dat_hang (
    ...
    trang_thai      ENUM('moi', 'da_xac_nhan', 'dang_phuc_vu', 'hoan_tat', 'huy') DEFAULT 'moi',
    ghi_chu         TEXT,
    nguoi_tao       VARCHAR(50),
    ngay_tao        DATETIME DEFAULT CURRENT_TIMESTAMP,
    ngay_cap_nhat   DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE loai_khuyen_mai (
    id              INT AUTO_INCREMENT PRIMARY KEY,
    ten_loai        VARCHAR(100) NOT NULL COMMENT 'VD: Giảm giá % theo SP, Giảm giá theo nhóm hàng, Giảm giá tổng bill, Mua X tặng Y'
) ENGINE=InnoDB;
...
CREATE TABLE khuyen_mai_chi_tiet (
    ...
    so_luong_mua    INT DEFAULT 0 COMMENT 'Số lượng mua để được khuyến mại',
    so_luong_tang   INT DEFAULT 0 COMMENT 'Số lượng tặng',
...
CREATE TABLE the_tra_truoc (
    id              INT AUTO_INCREMENT PRIMARY KEY,
    ma_the          VARCHAR(50) NOT NULL UNIQUE COMMENT 'VD: 123456',
    nhom_the_id     INT,
    khach_hang_id   INT,
    so_du           DECIMAL(15,2) DEFAULT 0,
    khoa            TINYINT DEFAULT 0 COMMENT '0=Mở, 1=Khóa',
    ngay_het_han    DATE,
    ghi_chu         TEXT,
    ngay_tao        DATETIME DEFAULT CURRENT_TIMESTAMP,
    nguoi_tao       VARCHAR(50),
...
CREATE TABLE the_tra_truoc_lich_su (
    id              INT AUTO_INCREMENT PRIMARY KEY,
    the_id          INT NOT NULL,
    loai_giao_dich  ENUM('nap', 'su_dung', 'hoan') NOT NULL,
    so_tien         DECIMAL(15,2) NOT NULL,
...
CREATE TABLE kho_hang (
    id              INT AUTO_INCREMENT PRIMARY KEY,
    ten_kho         VARCHAR(100) NOT NULL COMMENT 'VD: KHO BÁN HÀNG',
    cua_hang_id     INT,
    dia_chi         VARCHAR(500),
    ghi_chu         TEXT,
    trang_thai      TINYINT DEFAULT 1,
...
CREATE TABLE phieu_nhap_kho (
    ...
    dien_giai       TEXT,
    ghi_chu         TEXT,
    trang_thai      ENUM('nhap', 'da_duyet', 'huy') DEFAULT 'nhap',
    ngay_tao        DATETIME DEFAULT CURRENT_TIMESTAMP,
    nguoi_tao       VARCHAR(50),
...
CREATE TABLE phieu_xuat_kho (
    ...
    nhan_vien_id    INT,
    cua_hang_id     INT,
    ly_do_xuat      VARCHAR(200) COMMENT 'Xuất bán, xuất hủy, xuất chuyển kho...',
    tong_tien       DECIMAL(15,2) DEFAULT 0,
    dien_giai       TEXT,
    ghi_chu         TEXT,
    trang_thai      ENUM('nhap', 'da_duyet', 'huy') DEFAULT 'nhap',
...
CREATE TABLE phieu_kiem_ke (
    id              INT AUTO_INCREMENT PRIMARY KEY,
    so_phieu        VARCHAR(50) NOT NULL,
    ngay_kiem_ke    DATETIME DEFAULT CURRENT_TIMESTAMP,
    kho_id          INT NOT NULL,
    nhan_vien_id    INT,
    cua_hang_id     INT,
    dien_giai       TEXT,
    ghi_chu         TEXT,
    trang_thai      ENUM('dang_kiem', 'hoan_tat', 'huy') DEFAULT 'dang_kiem',
    thanh_toan      DECIMAL(15,2) DEFAULT 0,
    con_lai         DECIMAL(15,2) DEFAULT 0,
    ngay_tao        DATETIME DEFAULT CURRENT_TIMESTAMP,
    nguoi_tao       VARCHAR(50),
...
CREATE TABLE phieu_kiem_ke_chi_tiet (
    ...
    mat_hang_id     INT NOT NULL,
    ton_he_thong    DECIMAL(15,4) DEFAULT 0 COMMENT 'Tồn kho trên hệ thống',
    ton_thuc_te     DECIMAL(15,4) DEFAULT 0 COMMENT 'Tồn kho thực tế kiểm đếm',
    chenh_lech      DECIMAL(15,4) DEFAULT 0 COMMENT 'Chênh lệch = thực tế - hệ thống',
    dvt_id          INT,
    ghi_chu         VARCHAR(500),
...
CREATE TABLE ton_kho (
    ...
    mat_hang_id     INT NOT NULL,
    kho_id          INT NOT NULL,
    so_luong_ton    DECIMAL(15,4) DEFAULT 0 COMMENT 'Tổng tồn kho',
    gia_von_tb      DECIMAL(15,2) DEFAULT 0 COMMENT 'Giá vốn trung bình',
    quy_doi         DECIMAL(15,4) DEFAULT 1 COMMENT 'Hệ số quy đổi DVT',
    ton_2_dvt       DECIMAL(15,4) DEFAULT 0 COMMENT 'Tồn quy đổi theo DVT 2',
    ngay_cap_nhat   DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    UNIQUE KEY uk_mathang_kho (mat_hang_id, kho_id),
    FOREIGN KEY (mat_hang_id) REFERENCES mat_hang(id),
    FOREIGN KEY (kho_id) REFERENCES kho_hang(id)
) ENGINE=InnoDB;
...
CREATE TABLE cong_no_khach_hang (
    ...
    dien_giai       TEXT,
    luy_ke          DECIMAL(15,2) DEFAULT 0 COMMENT 'Lũy kế công nợ',
    trang_thai      ENUM('con_no', 'da_thanh_toan') DEFAULT 'con_no',
    FOREIGN KEY (khach_hang_id) REFERENCES khach_hang(id),
    FOREIGN KEY (hoa_don_id) REFERENCES hoa_don(id)
) ENGINE=InnoDB;
...
CREATE TABLE ly_do_thu_chi (
    id              INT AUTO_INCREMENT PRIMARY KEY,
    ten_ly_do       VARCHAR(200) NOT NULL COMMENT 'VD: Chi lương NV, Đặt trước, Đồ dùng, Tiền điện, Tiền nhà...',
...
CREATE TABLE tai_khoan_ngan_hang (
    id              INT AUTO_INCREMENT PRIMARY KEY,
    ten_tai_khoan   VARCHAR(100) NOT NULL COMMENT 'VD: tk1, tk2, tk3',
...
CREATE TABLE phieu_thu (
    ...
    ngay            DATETIME DEFAULT CURRENT_TIMESTAMP,
    ly_do_id        INT,
    doi_tuong       VARCHAR(200) COMMENT 'Tên khách hàng/nhà cung cấp',
    dien_giai       TEXT,
    chung_tu_goc    VARCHAR(100) COMMENT 'Số chứng từ gốc',
    so_tien         DECIMAL(15,2) NOT NULL DEFAULT 0,
    ghi_chu         TEXT,
    chuyen_khoan    TINYINT DEFAULT 0,
    dat_hang_id     INT COMMENT 'Liên kết đặt hàng',
...
CREATE TABLE nhat_ky_hoat_dong (
    ...
    noi_dung        TEXT NOT NULL COMMENT 'VD: Mở hóa đơn trên bàn Bàn 01, Đặt khách hàng Anh Chung...',
    tai_khoan       VARCHAR(50),
    thiet_bi        VARCHAR(100) COMMENT 'VD: LAPTOP-6KP13OSJ',
    ban             VARCHAR(50),
    chuc_nang       VARCHAR(100) COMMENT 'VD: Sử dụng dịch vụ, Đặt bàn...',
    ngay_tao        DATETIME DEFAULT CURRENT_TIMESTAMP,
    INDEX idx_ngay (ngay),
...
CREATE OR REPLACE VIEW v_thong_ke_doanh_thu AS
SELECT
...
GROUP BY DATE(hd.ngay_tao), hd.cua_hang_id;

-- View: Thống kê mặt hàng bán
CREATE OR REPLACE VIEW v_thong_ke_mat_hang_ban AS
SELECT
    DATE(hd.ngay_tao) AS ngay,
    hdct.mat_hang_id,
    mh.ma_hang,
...
WHERE hd.trang_thai = 'da_thanh_toan'
GROUP BY DATE(hd.ngay_tao), hdct.mat_hang_id, mh.ma_hang, mh.ten_hang,
         dvt.ten_dvt, hdct.don_gia, mh.gia_von;

-- View: Tồn kho nhiều kho
CREATE OR REPLACE VIEW v_ton_nhieu_kho AS
SELECT
...
-- View: Tồn quỹ
CREATE OR REPLACE VIEW v_ton_quy AS
SELECT
