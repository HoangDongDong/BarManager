...
{
    public partial class TaoPhieuThuWindow : Window
    {
        ...
        public TaoPhieuThuWindow(string id = null)
        {
            InitializeComponent();
            _id = id;
        ...
        private async Task LoadAllPhieuListAsync()
        {
            try
            {
                _allPhieuList = await LocalPhieuThuChiService.GetAllPhieuListAsync(isThu: true);
                if (!string.IsNullOrEmpty(_id))
                {
        ...
        {
            _id = null;
            Title = "PHIẾU THU - THÊM MỚI";
            DpNgay.SelectedDate = DateTime.Today;
            TxtSoPhieu.Text = await LocalPhieuThuChiService.GetNextSoPhieuAsync(isThu: true);
            TxtChungTuGoc.Text = "";
        ...
        private async Task LoadPhieuDetailAsync(string id)
        {
            try
            {
                ...
                if (p != null)
                {
                    ...
                    TxtSoTien.Text = p.Thu.HasValue ? p.Thu.Value.ToString("N0", CultureInfo.InvariantCulture) : "0";
                    ...
                    if (!string.IsNullOrEmpty(p.DlydothuchiId))
                    {
                        ...
                        CboLyDoThu.SelectedValue = p.DlydothuchiId;
                    }
        ...
        {
            if (CboPhanLoai.SelectedItem != null && CboLyDoThu.SelectedItem != CboPhanLoai.SelectedItem)
            {
                CboLyDoThu.SelectedItem = CboPhanLoai.SelectedItem;
            }
        }
        ...
        private async Task<bool> SaveDataAsync()
        {
            ...
            {
                MessageBox.Show("Vui lòng nhập số phiếu thu!", "Cảnh báo", MessageBoxButton.OK, MessageBoxImage.Warning);
                TxtSoPhieu.Focus();
                return false;
            }
            ...
            {
                MessageBox.Show("Vui lòng nhập số tiền thu hợp lệ (> 0)!", "Cảnh báo", MessageBoxButton.OK, MessageBoxImage.Warning);
                TxtSoTien.Focus();
                return false;
            }
            ...
            {
                MessageBox.Show("Lỗi lưu phiếu thu: " + error, "Lỗi", MessageBoxButton.OK, MessageBoxImage.Error);
                return false;
            }
        }
        ...
        private async void BtnLuu_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                MessageBox.Show("Lưu phiếu thu thành công!", "Thông báo", MessageBoxButton.OK, MessageBoxImage.Information);
                DialogResult = true;
            }
        }
        ...
        private async void BtnLuuVaIn_Click(object sender, RoutedEventArgs e)
        {
            if (await SaveDataAsync())
            {
                MessageBox.Show($"Lưu thành công!\nĐang gửi lệnh in phiếu thu '{TxtSoPhieu.Text}'...", "In phiếu thu", MessageBoxButton.OK, MessageBoxImage.Information);
                DialogResult = true;
                Close();
            }
        }
        ...
        private async void BtnSaoChep_Click(object sender, RoutedEventArgs e)
        {
            _id = null;
            TxtSoPhieu.Text = await LocalPhieuThuChiService.GetNextSoPhieuAsync(isThu: true);
            Title = "PHIẾU THU - THÊM MỚI (Sao chép)";
