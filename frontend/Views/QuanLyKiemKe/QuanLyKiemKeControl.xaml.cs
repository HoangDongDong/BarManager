...
using QuanLyBar.Client.Views.QuanLyNhapKho;

namespace QuanLyBar.Client.Views.QuanLyKiemKe
{
    public partial class QuanLyKiemKeControl : UserControl
    {
        private ObservableCollection<KhoHangTreeItem> _treeItems = new ObservableCollection<KhoHangTreeItem>();
        private List<PhieuKiemKeItem> _allPhieuKiemKe = new List<PhieuKiemKeItem>();
        private PhieuKiemKeItem _selectedPhieuKiemKe;
        private KhoHangTreeItem _selectedTreeItem;

        public QuanLyKiemKeControl()
        {
            InitializeComponent();
            Loaded += QuanLyKiemKeControl_Loaded;
            PreviewKeyDown += QuanLyKiemKeControl_PreviewKeyDown;
        }

        private async void QuanLyKiemKeControl_Loaded(object sender, RoutedEventArgs e)
        {
        ...
        public async Task LoadDataGridAsync()
        {
            try
            {
                ...
                if (_selectedTreeItem != null)
                {
                    ...
                    else if (_selectedTreeItem.Id != "ALL" && _selectedTreeItem.Id != "UNASSIGNED")
                    {
                        switch (_currentTreeMode)
                        {
                            case "KhoNhap":
                ...
                _allPhieuKiemKe = await LocalKiemKeService.GetPhieuKiemKeListAsync(
                    tuNgay, denNgay, khoNhapId, nvId, cuaHangId, taiKhoanNganHangId, bangGiaId, status);
        ...
        private void ApplyFilter()
        {
            ...
            var filtered = _allPhieuKiemKe;

            if (!string.IsNullOrEmpty(kw))
            {
                filtered = filtered.Where(x =>
                    (x.SoPhieu?.ToLowerInvariant().Contains(kw) == true) ||
                    (x.TenKhoHang?.ToLowerInvariant().Contains(kw) == true) ||
                    (x.TenNhanVien?.ToLowerInvariant().Contains(kw) == true) ||
                    (x.DienGiai?.ToLowerInvariant().Contains(kw) == true) ||
                    (x.Note?.ToLowerInvariant().Contains(kw) == true) ||
                    (x.TenCuaHang?.ToLowerInvariant().Contains(kw) == true)
                ).ToList();
            }
            ...
            DgPhieuKiemKe.ItemsSource = filtered;

            if (filtered.Count > 0)
            {
                DgPhieuKiemKe.SelectedIndex = 0;
            }
            else
            {
                _selectedPhieuKiemKe = null;
                ClearBottomTabs();
            }
        }
        ...
        private void DgPhieuKiemKe_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            _selectedPhieuKiemKe = DgPhieuKiemKe.SelectedItem as PhieuKiemKeItem;
            if (_selectedPhieuKiemKe != null)
            {
                _ = UpdateBottomTabsAsync(_selectedPhieuKiemKe);
            }
            else
            {
                ClearBottomTabs();
        ...
        private void DgPhieuKiemKe_MouseDoubleClick(object sender, MouseButtonEventArgs e)
        {
            if (_selectedPhieuKiemKe != null)
            {
        ...
        private void ClearBottomTabs()
        {
            ...
            DgKiemKeChiTietSub.ItemsSource = null;
        ...
        private async Task UpdateBottomTabsAsync(PhieuKiemKeItem item)
        {
            if (item == null)
            {
                ClearBottomTabs();
            ...
            try
            {
                var details = await LocalKiemKeService.GetPhieuKiemKeChiTietAsync(item.Id);
                DgKiemKeChiTietSub.ItemsSource = details;
            }
            catch (Exception ex)
            {
        ...
        private void BtnSuaKho_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                MessageBox.Show("Vui lòng chọn một mục cụ thể trên cây để chỉnh sửa!", "Thông báo", MessageBoxButton.OK, MessageBoxImage.Information);
                return;
            }
            ...
            switch (_currentTreeMode)
            {
                case "NhanVienNhap":
                    {
                        var win = new ThemNhanVienWindow(_selectedTreeItem.Id);
                        win.Owner = Window.GetWindow(this);
                        win.OnSaved += async () => { await LoadTreeAsync(); await LoadLookupsAsync(); };
                        win.ShowDialog();
                    }
        ...
        private void BtnQuickDate_Click(object sender, RoutedEventArgs e)
        {
            ...
            m1.Click += (s, ev) => { DpTuNgay.SelectedDate = DateTime.Today; DpDenNgay.SelectedDate = DateTime.Today; };
            var m2 = new MenuItem { Header = "Hôm qua" };
            m2.Click += (s, ev) => { DpTuNgay.SelectedDate = DateTime.Today.AddDays(-1); DpDenNgay.SelectedDate = DateTime.Today.AddDays(-1); };
            var m3 = new MenuItem { Header = "Tuần này" };
            m3.Click += (s, ev) => { DpTuNgay.SelectedDate = DateTime.Today.AddDays(-(int)DateTime.Today.DayOfWeek + 1); DpDenNgay.SelectedDate = DateTime.Today; };
            var m4 = new MenuItem { Header = "Tháng này" };
            m4.Click += (s, ev) => { DpTuNgay.SelectedDate = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1); DpDenNgay.SelectedDate = DateTime.Today; };
            var m5 = new MenuItem { Header = "Tháng trước" };
            m5.Click += (s, ev) =>
            {
                var first = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1).AddMonths(-1);
                var last = first.AddMonths(1).AddDays(-1);
                DpTuNgay.SelectedDate = first;
                DpDenNgay.SelectedDate = last;
            };
        ...
        private void BtnThemMoi_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                var win = new ThemPhieuKiemKeWindow(allPhieuKiemKe: _allPhieuKiemKe);
            ...
            {
                MessageBox.Show("Lỗi mở màn hình Thêm mới phiếu kiểm kê: " + ex.Message, "Lỗi", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void BtnChinhSua_Click(object sender, RoutedEventArgs e)
        {
            if (_selectedPhieuKiemKe == null)
            {
                MessageBox.Show("Vui lòng chọn một phiếu kiểm kê để chỉnh sửa!", "Thông báo", MessageBoxButton.OK, MessageBoxImage.Information);
                return;
            }

            try
            {
                var win = new ThemPhieuKiemKeWindow(_selectedPhieuKiemKe.Id, _allPhieuKiemKe);
                var parentWin = Window.GetWindow(this);
                if (parentWin != null) win.Owner = parentWin;
                win.OnSaved += async () =>
                {
            ...
            {
                MessageBox.Show("Lỗi mở màn hình Chỉnh sửa phiếu kiểm kê: " + ex.Message, "Lỗi", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private async void BtnXoa_Click(object sender, RoutedEventArgs e)
        {
            if (_selectedPhieuKiemKe == null)
            {
                MessageBox.Show("Vui lòng chọn một phiếu kiểm kê cần xóa!", "Thông báo", MessageBoxButton.OK, MessageBoxImage.Information);
                return;
            }

            bool isTrash = (_selectedTreeItem?.Id == "TRASH");
            string confirmMsg = isTrash
                ? $"Bạn có chắc chắn muốn XÓA VĨNH VIỄN phiếu kiểm kê '{_selectedPhieuKiemKe.SoPhieu}' khỏi hệ thống?"
                : $"Bạn có chắc chắn muốn chuyển phiếu kiểm kê '{_selectedPhieuKiemKe.SoPhieu}' vào Thùng rác?";

            var res = MessageBox.Show(confirmMsg, "Xác nhận xóa", MessageBoxButton.YesNo, MessageBoxImage.Question);
            if (res == MessageBoxResult.Yes)
            {
                bool success = await LocalKiemKeService.DeletePhieuKiemKeAsync(_selectedPhieuKiemKe.Id, isTrash);
                if (success)
                {
                    MessageBox.Show("Đã xóa phiếu kiểm kê thành công!", "Thông báo", MessageBoxButton.OK, MessageBoxImage.Information);
                    await LoadDataGridAsync();
                }
                else
                {
                    MessageBox.Show("Không thể xóa phiếu kiểm kê. Vui lòng thử lại!", "Lỗi", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
        }
        ...
        {
            MessageBox.Show("Chức năng thêm phiếu kiểm kê từ Excel đang được phát triển!", "Thông báo", MessageBoxButton.OK, MessageBoxImage.Information);
        }
        ...
        private void BtnXuatExcel_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                MessageBox.Show("Không có dữ liệu phiếu kiểm kê để xuất Excel!", "Thông báo", MessageBoxButton.OK, MessageBoxImage.Information);
                return;
            }
            ...
            try
            {
                var sfd = new SaveFileDialog
                {
                    ...
                    FileName = $"DanhSachPhieuKiemKe_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx"
                };
                ...
                if (sfd.ShowDialog() == true)
                {
                    ...
                    {
                        var ws = wb.Worksheets.Add("PhieuKiemKe");
                        string[] headers = new[] { "STT", "Số phiếu", "Ngày", "Kho hàng", "Diễn giải", "Nhân viên", "Ghi chú", "Cửa hàng", "Tổng cộng" };
                        ...
                        int r = 2;
                        foreach (var item in _allPhieuKiemKe)
                        {
                            ws.Cell(r, 1).Value = item.Stt;
                            ws.Cell(r, 2).Value = item.SoPhieu;
                            ws.Cell(r, 3).Value = item.NgayHienThi;
                            ws.Cell(r, 4).Value = item.TenKhoHang;
                            ws.Cell(r, 5).Value = item.DienGiai;
                            ws.Cell(r, 6).Value = item.TenNhanVien;
                            ws.Cell(r, 7).Value = item.Note;
                            ws.Cell(r, 8).Value = item.TenCuaHang;
                            ws.Cell(r, 9).Value = item.TongCong;
                            r++;
                        }
                    ...
                    MessageBox.Show($"Đã xuất thành công {_allPhieuKiemKe.Count} phiếu kiểm kê ra file Excel!", "Xuất Excel", MessageBoxButton.OK, MessageBoxImage.Information);
                }
            }
            catch (Exception ex)
            {
        ...
        {
            var win = new InLuoiWindow(DgPhieuKiemKe, "Phiếu kiểm kê kho");
            win.Owner = Window.GetWindow(this);
            win.ShowDialog();
        }
        ...
        {
            if (_allPhieuKiemKe.Count == 0)
            {
                MessageBox.Show("Không có dữ liệu phiếu kiểm kê!", "Thông báo", MessageBoxButton.OK, MessageBoxImage.Information);
                return;
            }

            decimal tongCong = _allPhieuKiemKe.Sum(x => x.TongCong);
            MessageBox.Show($"Tổng cộng giá trị chênh lệch kiểm kê ({_allPhieuKiemKe.Count} phiếu): {tongCong:N0} VNĐ", "Tổng cộng", MessageBoxButton.OK, MessageBoxImage.Information);
        }

        private void BtnPhanTich_Click(object sender, RoutedEventArgs e)
        {
            MessageBox.Show("Chức năng phân tích dữ liệu kiểm kê đang được phát triển!", "Thông báo", MessageBoxButton.OK, MessageBoxImage.Information);
        }
        ...
        private void DataGridRow_PreviewMouseRightButtonDown(object sender, MouseButtonEventArgs e)
        {
            if (sender is DataGridRow row)
            {
                if (!row.IsSelected)
                {
                    DgPhieuKiemKe.SelectedItems.Clear();
                    row.IsSelected = true;
                }
                row.Focus();
        ...
        {
            if (_selectedPhieuKiemKe == null) return;
            bool ok = await LocalKiemKeService.RestorePhieuKiemKeAsync(_selectedPhieuKiemKe.Id);
            if (ok)
            {
                await LoadDataGridAsync();
            }
        ...
        {
            var list = DgPhieuKiemKe.ItemsSource as List<PhieuKiemKeItem>;
            if (list != null) DgPhieuKiemKe.ItemsSource = list.OrderBy(x => x.SoPhieu).ToList();
        }

        private void MenuItem_SortDesc_Click(object sender, RoutedEventArgs e)
        {
            var list = DgPhieuKiemKe.ItemsSource as List<PhieuKiemKeItem>;
            if (list != null) DgPhieuKiemKe.ItemsSource = list.OrderByDescending(x => x.SoPhieu).ToList();
        }
        ...
        {
            var list = DgPhieuKiemKe.ItemsSource as List<PhieuKiemKeItem>;
            if (list != null) DgPhieuKiemKe.ItemsSource = list.OrderByDescending(x => x.Ngay).ToList();
        }

        private void MenuItem_SortByKho_Click(object sender, RoutedEventArgs e)
        {
            var list = DgPhieuKiemKe.ItemsSource as List<PhieuKiemKeItem>;
            if (list != null) DgPhieuKiemKe.ItemsSource = list.OrderBy(x => x.TenKhoHang).ToList();
        }

        private void MenuItem_SortByNv_Click(object sender, RoutedEventArgs e)
        {
            var list = DgPhieuKiemKe.ItemsSource as List<PhieuKiemKeItem>;
            if (list != null) DgPhieuKiemKe.ItemsSource = list.OrderBy(x => x.TenNhanVien).ToList();
        }

        private void MenuItem_SortByTongCong_Click(object sender, RoutedEventArgs e)
        {
            var list = DgPhieuKiemKe.ItemsSource as List<PhieuKiemKeItem>;
            if (list != null) DgPhieuKiemKe.ItemsSource = list.OrderByDescending(x => x.TongCong).ToList();
        }
        ...
        private void MenuItem_SaoChepO_Click(object sender, RoutedEventArgs e)
        {
            if (_selectedPhieuKiemKe != null)
            {
                Clipboard.SetText(_selectedPhieuKiemKe.SoPhieu);
            }
        }
        ...
        private void MenuItem_SaoChepVungChon_Click(object sender, RoutedEventArgs e)
        {
            if (_selectedPhieuKiemKe != null)
            {
                Clipboard.SetText($"{_selectedPhieuKiemKe.SoPhieu}\t{_selectedPhieuKiemKe.NgayHienThi}\t{_selectedPhieuKiemKe.TenKhoHang}\t{_selectedPhieuKiemKe.TongCong:N0}");
            }
        }

        private void MenuItem_TuDongGianCot_Click(object sender, RoutedEventArgs e)
        {
            foreach (var col in DgPhieuKiemKe.Columns)
            {
        ...
        {
            var win = new InLuoiWindow(DgPhieuKiemKe, "Phiếu kiểm kê");
            win.Owner = Window.GetWindow(this);
            win.ShowDialog();
        }
        ...
        private void QuanLyKiemKeControl_PreviewKeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Insert)
            {
