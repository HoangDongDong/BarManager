...
{
    public partial class TinhLuongControl : UserControl
    {
        ...
        public async Task LoadBangLuongListAsync(string selectId = null)
        {
            try
            {
                ...
                if (_bangLuongList.Count > 0)
                {
                    ...
                    await LoadTinhLuongMatrixAsync(_selectedBangLuong);
                }
                else
                {
                    ...
                    _matrixRows.Clear();
                    DgTinhLuong.ItemsSource = null;
                    DgTinhLuong.Columns.Clear();
                    UpdateTotalsSummary();
                }
            }
            catch (Exception ex)
        ...
        private async Task LoadTinhLuongMatrixAsync(BangLuongItemViewModel bangLuong)
        {
            ...
            try
            {
                ...
                DgTinhLuong.ItemsSource = null;
                DgTinhLuong.ItemsSource = _matrixRows;
                UpdateTotalsSummary();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Lỗi tải bảng tính lương: " + ex.Message, "Lỗi", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void BuildGridColumns(int month, int year)
        {
            ...
            nameCellStyle.Setters.Add(new Setter(DataGridCell.BackgroundProperty, new SolidColorBrush(Color.FromRgb(212, 232, 252))));
            ...
            DgTinhLuong.Columns.Add(colName);
            ...
            for (int d = 1; d <= daysInMonth; d++)
            {
                ...
                string dayOfWeekStr = date.ToString("ddd", new CultureInfo("vi-VN"));
                ...
                DgTinhLuong.Columns.Add(colDay);
            }

            // 4. Các cột tính lương (Lương cơ bản, Tổng lương, Thưởng, Phạt, Tạm ứng, Thực nhận)
            // Lương cơ bản
            DgTinhLuong.Columns.Add(CreateSalaryColumn("Lương cơ bản", "LuongCoBanDisplay", 95, Brushes.Black, false));

            // Tổng lương
            DgTinhLuong.Columns.Add(CreateSalaryColumn("Tổng lương", "TongLuongDisplay", 95, new SolidColorBrush(Color.FromRgb(0, 85, 170)), true));

            // Thưởng
            DgTinhLuong.Columns.Add(CreateSalaryColumn("Thưởng", "ThuongDisplay", 80, new SolidColorBrush(Color.FromRgb(39, 174, 96)), false));

            // Phạt
            DgTinhLuong.Columns.Add(CreateSalaryColumn("Phạt", "PhatDisplay", 80, new SolidColorBrush(Color.FromRgb(231, 76, 60)), false));

            // Tạm ứng
            DgTinhLuong.Columns.Add(CreateSalaryColumn("Tạm ứng", "TamUngDisplay", 85, new SolidColorBrush(Color.FromRgb(211, 84, 0)), false));

            // Thực nhận
            DgTinhLuong.Columns.Add(CreateSalaryColumn("Thực nhận", "ThucNhanDisplay", 100, new SolidColorBrush(Color.FromRgb(192, 57, 43)), true, new SolidColorBrush(Color.FromRgb(255, 251, 240))));
        }

        private static DataGridTextColumn CreateSalaryColumn(string header, string bindingPath, double width, Brush fgBrush, bool isBold = false, Brush bgBrush = null)
        {
            ...
            elemStyle.Setters.Add(new Setter(TextBlock.TextAlignmentProperty, TextAlignment.Right));
            elemStyle.Setters.Add(new Setter(TextBlock.PaddingProperty, new Thickness(4, 0, 6, 0)));
            elemStyle.Setters.Add(new Setter(TextBlock.FontSizeProperty, 11.5));
            if (isBold) elemStyle.Setters.Add(new Setter(TextBlock.FontWeightProperty, FontWeights.Bold));
            if (fgBrush != null) elemStyle.Setters.Add(new Setter(TextBlock.ForegroundProperty, fgBrush));
            col.ElementStyle = elemStyle;

            var cellStyle = new Style(typeof(DataGridCell));
            if (bgBrush != null)
                cellStyle.Setters.Add(new Setter(DataGridCell.BackgroundProperty, bgBrush));
            else
                cellStyle.Setters.Add(new Setter(DataGridCell.BackgroundProperty, Brushes.White));
            cellStyle.Setters.Add(new Setter(DataGridCell.BorderBrushProperty, new SolidColorBrush(Color.FromRgb(200, 215, 230))));
        ...
        private void DgTinhLuong_PreviewMouseRightButtonDown(object sender, MouseButtonEventArgs e)
        {
            var hit = VisualTreeHelper.HitTest(DgTinhLuong, e.GetPosition(DgTinhLuong));
            if (hit != null)
            {
        ...
        {
            var selectedCells = DgTinhLuong.SelectedCells;
        ...
        private void UpdateTotalsSummary()
        {
            if (_matrixRows == null || _matrixRows.Count == 0)
            {
                ...
                TxtTotalTongLuong.Text = "0 VNĐ";
                TxtTotalThucNhan.Text = "0 VNĐ";
                return;
            }
            ...
            decimal sumTongLuong = firstRows.Sum(x => x.TongLuong);
            decimal sumThucNhan = firstRows.Sum(x => x.ThucNhan);

            TxtTotalTongLuong.Text = $"{sumTongLuong:N0} VNĐ";
            TxtTotalThucNhan.Text = $"{sumThucNhan:N0} VNĐ";
        }
        ...
        private async void BtnRefreshLuong_Click(object sender, RoutedEventArgs e)
        {
            if (_selectedBangLuong == null)
            {
                MessageBox.Show("Vui lòng chọn bảng lương để refresh dữ liệu tính lương!", "Thông báo", MessageBoxButton.OK, MessageBoxImage.Information);
                return;
            }

            await LoadTinhLuongMatrixAsync(_selectedBangLuong);
            MessageBox.Show("Đã nạp và tính lại toàn bộ lương, thưởng, phạt, tạm ứng thành công!", "Thông báo", MessageBoxButton.OK, MessageBoxImage.Information);
        }

        private void BtnChiLuong_Click(object sender, RoutedEventArgs e)
        {
            if (_selectedBangLuong == null || _matrixRows == null || _matrixRows.Count == 0)
            {
                MessageBox.Show("Vui lòng chọn bảng lương có dữ liệu để chi lương!", "Thông báo", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }

            var firstRows = _matrixRows.Where(x => x.IsFirstShiftOfEmployee).ToList();
            decimal totalThucNhan = firstRows.Sum(x => x.ThucNhan);

            var res = MessageBox.Show(
                $"Bạn có muốn lập phiếu chi trả lương cho bảng lương '{_selectedBangLuong.Name}'?\n\n" +
                $"• Số lượng nhân viên: {firstRows.Count}\n" +
                $"• Tổng thực nhận: {totalThucNhan:N0} VNĐ",
                "Xác nhận chi lương",
            ...
            {
                var phieuChiWin = new QuanLyBar.Client.Views.PhieuThuChi.TaoPhieuChiWindow();
                phieuChiWin.Owner = Window.GetWindow(this);
                phieuChiWin.ShowDialog();
            }
        }
        ...
        private async void BtnCapNhat_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (ok)
            {
                MessageBox.Show($"Đã cập nhật bảng chấm công và tổng hợp lương cho '{_selectedBangLuong.Name}' thành công!", "Thông báo", MessageBoxButton.OK, MessageBoxImage.Information);
                await LoadTinhLuongMatrixAsync(_selectedBangLuong);
            }
            else
            {
