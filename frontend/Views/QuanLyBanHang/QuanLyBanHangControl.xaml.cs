            ...
            if (DgHoaDon.SelectedItem is HoaDonViewModel selectedHoaDon)
            {
                try
                {
                    if (int.TryParse(selectedHoaDon.Id, out int donHangId))
                    {
                        ...
                        {
                            tienHang += (ct.DonGia * ct.SoLuong);
                            tienGiamGia += (ct.DonGia * ct.SoLuong * ct.PhanTramGiamGia / 100);
                        }
                        
                        txtTienHang.Text = tienHang.ToString("N0");
                        txtTienGiamGia.Text = tienGiamGia.ToString("N0");
                        txtTongCong.Text = (tienHang - tienGiamGia).ToString("N0");
                        
                        if (tienHang > 0)
                            txtPhanTramGiamGia.Text = (tienGiamGia / tienHang * 100).ToString("N2");
                        else
