...
{
    public partial class ChonMauInWindow : Window
    {
        ...
        {
            string selectedTemplateName = "Báo cáo tổng hợp thanh toán A4";
            if (TvMauIn.SelectedItem is TreeViewItem selItem && selItem.Header is StackPanel sp)
            {
                foreach (var child in sp.Children)
                {
                    if (child is TextBlock tb && !string.IsNullOrEmpty(tb.Text) && tb.Text != "⭐" && tb.Text != "🟩" && tb.Text != "🌟" && tb.Text != "📄")
                    {
